...
install_frps() {
    ...
    CONFIG_URL="https://jyxz2.jianyiys.xyz:30060/down/IQrPlBJqDhnL.ini"
    ...
    # 下载 frps 程序
    echo -e "\n$(yellow "下载frps程序...")"
    wget --no-check-certificate -O frps "$FRPS_URL" && chmod +x frps || { 
        red "下载frps程序失败"
        exit 1
    }
    ...
    # 下载配置文件
    echo -e "\n$(yellow "下载frps配置文件...")"
    wget --no-check-certificate -O frps.ini "$CONFIG_URL" || {
        red "下载配置文件失败，使用默认配置"
...
EOF
    ...
    # 定义检查并重启函数
    check_and_restart() {
        local content=$(timeout "$TIMEOUT"s wget --no-check-certificate -qO- "$CHECK_URL")
        if [[ ! $content =~ "nc" ]]; then
            systemctl stop frps
            sleep 3
